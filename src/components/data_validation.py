...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe:pd.DataFrame)->bool:
        try:
            schema_no_of_columns = len(self.schema_config)
            logging.info(f"Required no of columns:{schema_no_of_columns}")
            
            ##test/train file columns
            logging.info(f"Dataframe has columns:{len(dataframe.columns)} ")
            
            ##check train/test file columns count = schema file columns count
            if len(dataframe.columns) == schema_no_of_columns:
                return True
            else:
                return False             
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold=0.05)->bool:
        try:
            ...
            for column in base_df.columns: ##iterate all columns in base_df i.e. train df
                d1 = base_df[column]
